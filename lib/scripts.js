...
$(document).ready(function(){
  ...
  var toDoArray = AllToDos.retrieve();
  var incompleteArray = AllToDos.filterComplete(toDoArray);
  AllToDos.renderStorage(incompleteArray);
});
...
$('.list-container').on('click', '.downvote', function(){
  // var importance = $(this).parent().parent().attr('importance');
...
$('.complete-btn').on('click', function(){
  toDoArray = AllToDos.retrieve();
  incompleteArray = AllToDos.filterComplete(toDoArray);
  completeArray = AllToDos.filterIncomplete(toDoArray);
  AllToDos.clearListContainer();
  AllToDos.renderStorage(incompleteArray);
  AllToDos.renderStorage(completeArray);
});
...
var AllToDos = {
  ...
  filterComplete: function() {
    var incompleteArray = this.toDoArray.filter(function (r) {
    return r.completeness === 'incomplete';
  });
  ...
  filterIncomplete: function() {
    var completeArray;
    completeArray = this.toDoArray.filter(function (r) {
    return r.completeness === 'complete';
  });
  },
  ...
  render: function(toDo) {
    $('.list-container').prepend('<div class="list-item ' + toDo.completeness + '"' +
    'id="' + toDo.id + '"><li class="title-style"><p class="new-title-input" contenteditable="true">' + toDo.title + '</p><button type="button" class="remove-button btn" ></button></li><li class="body-style"><p class="new-body-input" contenteditable="true">' + toDo.body + '</p></li><div class="importance-style"><button type="button" class="upvote btn"></button><button type="button" class="downvote btn"></button><p class="importance">importance: ' + '<span class="importance-value">' + toDo.importance + '</span>' + '<button class="complete-button">Completed Task</button></p></div></div>');
  },

  renderStorage: function(array) {
